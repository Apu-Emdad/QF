...
export function Header({ bottomNavEnabled = true }: IProps) {
  ...
  useEffect(() => {
    const checkWindowResize = () => {
      if (window.innerWidth < 900 && bottomNavEnabled) { setHideDefaultNav(true) }
      else { setHideDefaultNav(false) }
    };
  ...
  const shouldRenderBottomNav = (): boolean => {
    if (typeof window !== 'undefined') {
      if (window.innerWidth > 990 && (!bottomNavEnabled || !hideDefaultNav)) { return false }

      if (window.innerWidth < 990 && bottomNavEnabled && hideDefaultNav) { return true }
    }
